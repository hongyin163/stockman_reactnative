...
var macd={
    ...
    calculate: function (newData) {
        ...
        for (me.i; me.i < me.data.length; me.i++) {
            ...
            //
            var diffEma1 = me.diffEmaM[me.i - 1];
            var dea = me.getEma(diff, diffEma1, me.m, me.i);// diffEma1 * (m - 1) / (m + 1) + diff * (2 / (m + 1));
            me.diffEmaM.push(dea);

            bar = 2 * (diff - dea);

            macd.push([me.data[me.i][0],Number(diff.toFixed(2)),Number(dea.toFixed(2)),Number(bar.toFixed(2))]);
        }
        return macd;
    },
